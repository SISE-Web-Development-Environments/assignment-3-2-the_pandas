...
async function getUsersFromDb() {
  let users = await db_util.execQuery(
    "SELECT username, userPassword FROM dbo.Users"
  );
  return users;
}

async function getSpecificUserFromDbwithid(id) {
  let user = await db_util.execQuery(
    "SELECT username,user_id,userPassword FROM dbo.Users WHERE user_id='" +
      id +
      "'"
  );
  return user;
...
async function getSpecificUserFromDb(username) {
  let user = await db_util.execQuery(
    "SELECT username,user_id,userPassword FROM dbo.Users WHERE username='" +
      username +
      "'"
  );
  return user;
